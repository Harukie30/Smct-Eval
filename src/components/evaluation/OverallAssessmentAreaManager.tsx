...
export default function OverallAssessmentAreaManager({
  data,
  updateDataAction,
  employee,
  onSubmitAction,
  ...
  useEffect(() => {
    if (!isSubmittingEvaluation && showLoadingDialog) {
      // Add a small delay to ensure the dialog is visible for at least 1 second
      const timer = setTimeout(() => {
        setShowLoadingDialog(false);
      }, 1000);
      return () => clearTimeout(timer);
    }
  }, [isSubmittingEvaluation, showLoadingDialog]);
  ...
  // Adjusted weights for Area Manager evaluation
  const jobKnowledgeWeighted = (jobKnowledgeScore * 0.22).toFixed(2);
  const qualityOfWorkWeighted = (qualityOfWorkScore * 0.30).toFixed(2);
  const adaptabilityWeighted = (adaptabilityScore * 0.03).toFixed(2);
  const teamworkWeighted = (teamworkScore * 0.10).toFixed(2);
  const reliabilityWeighted = (reliabilityScore * 0.10).toFixed(2);
  const ethicalWeighted = (ethicalScore * 0.03).toFixed(2);
  const managerialSkillsWeighted = (managerialSkillsScore * 0.22).toFixed(2);
  ...
  `}
      ...
      {/* Loading Dialog */}
      <Dialog open={showLoadingDialog} onOpenChangeAction={() => {}}>
        <DialogContent className="sm:max-w-md p-6 relative">
