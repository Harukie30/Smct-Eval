...
export const UserProvider: React.FC<UserProviderProps> = ({ children }) => {
  ...
  const login = async (username: string, password: string  , rememberMe: boolean): Promise<any> => {
    try {
      setIsLoading(true);
      
      // Authenticate user using client data service
      const loginResult = await clientDataService.login(username, password, rememberMe);
