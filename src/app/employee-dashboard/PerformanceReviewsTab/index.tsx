...
export function PerformanceReviewsTab({ isActive = false, onViewEvaluation }: PerformanceReviewsTabProps) {
  ...
  const performanceMetrics = useMemo(() => {
    const ratings = submissions
      .map((s) => (s.evaluationData ? calculateOverallRating(s.evaluationData) : s.rating))
      .filter((r) => r > 0);
                  ...
                  {chartData.length === 0 ? (
                    <div className="h-64 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <div className="text-xs text-gray-400 mt-1">Complete your first evaluation to see trends</div>
                      </div>
                    </div>
                  ) : (
                    <div className="h-80">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      ...
                      <div className="text-sm text-gray-600 bg-white px-3 py-1 rounded-md border">
                        <span className="font-medium">
                          {submissions.filter(
                            (s) => (s.evaluationData ? calculateOverallRating(s.evaluationData) : s.rating) > 0
                          ).length}
                        </span>{' '}
                        evaluation
                        {submissions.filter(
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-semibold">{performanceMetrics.latestRating}</span>
                      <span className="text-sm text-gray-500">/5.0</span>
                      {performanceMetrics.trend !== 0 && (
                        ...
                        >
                          {performanceMetrics.trend > 0 ? '↗' : '↘'} {Math.abs(performanceMetrics.trend).toFixed(1)}
                        </Badge>
                      )}
                    </div>
                  </div>
