...
export function OverviewTab({
  ...
  const loadSubmissions = async () => {
    try {
      ...
      if (userSubmissions.length > 0) {
        ...
        lastSubmissionsTimestampRef.current = sortedByDate[0].submittedAt;
      }
    } catch (error) {
  ...
  // Automatic refresh when approval changes or new submissions are detected
  useEffect(() => {
    if (!profile?.email || !isActive) return;
    ...
    const checkForApprovalChanges = async () => {
      try {
        ...
        if (currentApprovedStr !== lastApprovedEvaluationsRef.current) {
          // Update the ref to prevent duplicate refreshes
          lastApprovedEvaluationsRef.current = currentApprovedStr;

          // Update state
          setApprovedEvaluations(new Set(currentApproved));

          // Automatically refresh submissions to show updated approval status
    ...
    const checkForNewSubmissions = async () => {
      try {
        ...
        // Check if count changed or if there's a newer submission
        const currentCount = finalSubmissions.length;
        const hasNewCount = currentCount !== lastSubmissionsCountRef.current;
        ...
        if (finalSubmissions.length > 0) {
          ...
          const latestTimestamp = sortedByDate[0].submittedAt;
          hasNewSubmission =
            latestTimestamp !== lastSubmissionsTimestampRef.current;
        }

        // If there are new submissions, refresh
        if (hasNewCount || hasNewSubmission) {
          await refreshSubmissions();
        }
      } catch (error) {
    ...
    const approvalIntervalId = setInterval(checkForApprovalChanges, 2000);
    const submissionsIntervalId = setInterval(checkForNewSubmissions, 2000);
    ...
    const handleStorageChange = (e: StorageEvent) => {
      ...
      // Check for submissions changes (localStorage key is 'submissions')
      if (e.key === "submissions" || e.key === "evaluationResults") {
        checkForNewSubmissions();
      }
    };
  ...
  const filteredSubmissions = useMemo(() => {
    ...
    return submissions.filter((submission) => {
      ...
      const acknowledgement = isEvaluationApproved(submission.id)
        ? "approved"
        : "pending";
              ...
              {filteredSubmissions.length > 4 && (
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 md:gap-0 mt-3 md:mt-4 px-4">
                  <div className="text-xs md:text-sm text-gray-600 order-2 sm:order-1">
                    ...
                    {Math.min(overviewEndIndex, filteredSubmissions.length)} of{" "}
