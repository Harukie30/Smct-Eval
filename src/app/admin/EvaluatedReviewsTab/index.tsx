...
// Helper function to get submission highlight
const getSubmissionHighlight = (submittedAt: string, id: number, approvalStatus?: string) => {
  const now = new Date();
  const hoursDiff = (now.getTime() - new Date(submittedAt).getTime()) / (1000 * 60 * 60);
  const isNew = hoursDiff <= 24;
  const isRecent = hoursDiff > 24 && hoursDiff <= 168; // 7 days
  const isFullyApproved = approvalStatus === 'fully_approved';
  
  let className = "hover:bg-gray-100 transition-colors cursor-pointer";
  let badge = null;
  let secondaryBadge = null;
  
  if (isFullyApproved) {
    className = "bg-green-50 hover:bg-green-100 border-l-4 border-l-green-500 transition-colors cursor-pointer";
    secondaryBadge = { text: '✓ Fully Approved', className: 'bg-green-100 text-green-800' };
  } else if (isNew) {
    className = "bg-yellow-50 hover:bg-yellow-100 border-l-4 border-l-yellow-500 transition-colors cursor-pointer";
    badge = { text: '⚡ NEW', className: 'bg-yellow-100 text-yellow-800' };
  } else if (isRecent) {
    className = "bg-blue-50 hover:bg-blue-100 border-l-4 border-l-blue-500 transition-colors cursor-pointer";
    badge = { text: '🕐 RECENT', className: 'bg-blue-100 text-blue-800' };
  }
  
  return { className, badge, secondaryBadge };
};
...
export function EvaluatedReviewsTab() {
  ...
  const filteredFeedbackData = useMemo(() => {
    ...
    let data = submissionsWithApprovalData.map((submission, index) => {
      ...
      return {
        ...
        overallRating: calculatedRating.toString()
      };
    });
  ...
  const handleViewEvaluation = async (feedback: any) => {
    try {
      // Fetch the full submission data
      const submission = await clientDataService.getSubmissionById(feedback.id);
      ...
      if (submission) {
        ...
        const submissionForModal = {
          id: submission.id,
          employeeName: submission.employeeName || feedback.employeeName,
          category: 'Performance Review',
          rating: ratingValue,
          submittedAt: submission.submittedAt || feedback.date,
          status: submission.status || 'completed',
          evaluator: submission.evaluator || feedback.reviewer,
          evaluationData: submission.evaluationData || feedback.evaluationData,
          employeeId: submission.employeeId,
          employeeEmail: submission.employeeEmail || feedback.employeeEmail,
          evaluatorId: submission.evaluatorId,
          evaluatorName: submission.evaluator || feedback.reviewer,
          period: submission.period,
          overallRating: submission.overallRating || ratingValue.toString(),
          approvalStatus: submission.approvalStatus || feedback.approvalStatus,
          employeeSignature: submission.employeeSignature || feedback.employeeSignature,
          employeeApprovedAt: submission.employeeApprovedAt || feedback.employeeApprovedAt,
          evaluatorSignature: submission.evaluatorSignature || feedback.evaluatorSignature,
          evaluatorApprovedAt: submission.evaluatorApprovedAt || feedback.evaluatorApprovedAt,
        };
  ...
  const handlePrintFeedback = (feedback: any) => {
    // Print functionality - can be implemented later
    window.print();
  };

  const handleDeleteClick = async (feedback: any) => {
    if (confirm(`Are you sure you want to delete the evaluation for ${feedback.employeeName}?`)) {
      try {
        await clientDataService.deleteSubmission(feedback.id);
        await loadSubmissions();
      } catch (error) {
                    ...
                    filteredFeedbackData.map((feedback) => {
                      ...
                      return (
                        <TableRow
                          ...
                          onClick={() => handleViewEvaluation(feedback)}
                        >
                          <TableCell className="px-6 py-3">
